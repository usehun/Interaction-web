(() => {
  ...
  const sceneInfo = [
    ...
    },
    {
      ...
      objs: {
        ...
        imagesPath: [
          "../src/img/blend-image-1.jpg",
          "../src/img/blend-image-2.jpg",
        ],
        images: [],
      },
      values: {
  ...
  function setCanvasImages() {
    ...
    for (let i = 0; i < sceneInfo[0].values.videoImagesCount; i++) {
      imgElem = new Image();
      // imgElem = document.createElement("img"); 위와 같다
      imgElem.src = `../src/video/IMG_${6726 + i}.JPG`;
    ...
    for (let i = 0; i < sceneInfo[2].values.videoImagesCount; i++) {
      imgElem2 = new Image();
      imgElem2.src = `../src/video2/IMG_${7027 + i}.JPG`;
  ...
  function calcValues(values, currentYOffset) {
    ...
    if (values.length === 3) {
      ...
      if (
        ...
        // 첫번째 인덱스와 두번째 인덱스의 비율
        rv =
          ((currentYOffset - partScrollStart) / partScrollHeight) *
            (values[1] - values[0]) +
          values[0];

        // console.log(((currentYOffset - partScrollStart) / partScrollHeight) * (values[1] - values[0]));

        // console.log(((currentYOffset - partScrollStart) / partScrollHeight) *(values[1] - values[0]) + values[0]);
  ...
  function playAnimation() {
    ...
    switch (currentScene) {
      case 0:
        // 변수를 사용하면 범위에 따른 실행이 아니라 전부 실행하기 때문에 불필요한 성능 사용
      ...
      case 2:
        ...
        if (scrollRatio > 0.9) {
          ...
          // document.body.offsetWidth 사용하는 이유는 스크롤 바가 폭넓이를 차지하고 있기 때문에 스크롤 바의 넓이를 포함하지 않는 폭 넓이
          // const recalculatedInnerHeight = window.innerWidth / canvasScaleRatio;
      ...
      case 3:
        ...
        if (!values.rectStartY) {
          // 스크롤이 빠르면 정확한 top 위치를 가져오지 못해 getBoundingClientRect().top 사용하지 않는다
  ...
  function scrollLoop() {
    ...
    if (enterNewScene) return; // enterNewScene true가 되면 함수를 리턴하고 끝낸다
